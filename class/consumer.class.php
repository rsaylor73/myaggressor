...
//print "SQL 2: $sql2<hr>";
   ...
   public function get_inventory($charterID,$bunks) {
	...
	if($_GET['passengers']==1)
		$sql=$sql."And `reserve`.`inventory`.`bunk_description` not like '%QN%' And `reserve`.`inventory`.`bunk_description` not like '%KING%' And `reserve`.`inventory`.`bunk_description` not like '%Queen%' and `reserve`.`inventory`.`bunk_description` not like '%Master%'";
      return $sql;
   }
...
//RBS//
   ...
   public function get_sex2($charter,$bunk) {
	...
	public function view_all_in_charter_and_type() {
		...
		if ($_SESSION['contact_type'] != "consumer") {
			...
			<table border=0 width=\"800\" cellspacing=\"3\">
				<tr>
					...
					<input type=\"image\" src=\"buttons/bt-bookall.png\" onclick=\"document.location.href='book_all.php?charter=$_GET[charter]&type=$_GET[type]&name=$_GET[name]&start_date=$_GET[start_date]&end_date=$_GET[end_date]&passengers=$_GET[passengers]$this_boats';return false;\">
		...
		/*
                                        <!--
                                        <input type=\"button\" value=\"Book All\" class=\"btn btn-success btn-lg\" style=\"width:117px;\" style=\"float: left;\"
